...
%         fprintf('Iterative ')
  %  tic()
%  dvds = 0; dvg = 0;
   [dvds,dvg, Idx] = virtualSource(params, [Vds;Vgs;Vbs;Vdsi;Vs;Vd;Vg],type);
  % toc()
...
%    Rd = Rs;                                                 % d-terminal resistance [ohms] For symmetric source and drain Rd = Rs.
    ...
    Qinv = Qref.*log1p(eta0);

    %Transport equations
    vx0 = vxo;
    Vdsats = vx0 .* Leff./ mu;
    Vdsat = Vdsats .* ( 1.0 - FF ) + phit .* FF;   % Saturation drain voltage for current    
    Fsat=(Vdsi./Vdsat)./((1+(Vdsi./Vdsat).^beta).^(1./beta));      % Transition function from linear to saturation.
