function IdsRes = computeResidualIdsAD( params,mvsAD)
...
%     dir(s_smaller_than_d) = -1;
    ...
    Vtpcorr = Vt0 + gamma * (sqrt( (phib - Vbs))- sqrt(phib))- Vdsi * delta; % Calculated from extrinsic Vbs
    ...
    Vt0bs = Vt0 + gamma .* (sqrt(  ( phib - Vbscorr)) - sqrt( phib )); % Computed from corrected intrinsic Vbs
    Vt0bs0 = Vt0 + gamma .* (sqrt(  ( phib - Vbsi)) - sqrt( phib ));   % Computed from uncorrected intrinsic Vbs
    ...
    %Total drain current
    IdsRes = Qinv_corr .* vx0 .* Fsat .* W - IdsAD;
%     correctSign = sign(IdsRes.x);
%     indZeros = correctSign == 0;
%     correctSign(indZeros) = 1;
    IdsRes = IdsRes.*type.*dir;%.*correctSign;  % Coulombs/s = A
