...
biasVoltage = [VyD,VyS,VyG(1+lengthRuns:4*lengthRuns);     %Drain  (Vy)
                VgD,VgS,VgG(1+lengthRuns:4*lengthRuns);    %Gate   (Vg)
                VxD,VxS,VxG(1+lengthRuns:4*lengthRuns);    %Source (Vx)
                VbD,VbS,VbG(1+lengthRuns:4*lengthRuns)];   %Body   (Vb)
            

Id_ref = [IdD,IdS,IdG(1+lengthRuns:4*lengthRuns)];
...
while( abs(err-errPrev) > tol)
    ...
    %phi and gamma can not be negative
    if fitParams(6) < 0
        fitParams(6) = fitParamsPrev(6)/2;
    end
 %   if fitParams(5) < 0
 %       fitParams(5) = fitParamsPrev(5)/2;
 %   end
    if ((- fitParams(4) +(fitParams(5)/2 + sqrt(fitParams(6)))^2) < 0)
        fitParams(5) = 2*(sqrt(fitParams(4))-sqrt(fitParams(6)));
       % fitParams(4) = (fitParams(5)/2 + sqrt(fitParams(6)))^2;
    end
