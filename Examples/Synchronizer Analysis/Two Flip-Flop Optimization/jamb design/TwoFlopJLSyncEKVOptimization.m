...
while(norm(transistorWidths-transistorWidthPrev) > norm(wStart)/numTx*tol || i < 3)
    ...
    sync = TWO_FLOP_JAMB_SYNC('jambSync_test',transistorWidths,1);
     ...
     dataSync = nb.simulate(strcat('twoFlop_jambSync_iter',num2str(i)),timeSpan,tCrit,v0');
     ...
     nbAnalysis.bisectionAnalysis(strcat('twoFlop_jambSyncAnalysis_',num2str(i)),dataSync,uVcrit,tCrit);
    ...
    transistorWidths(1:25) = pullDownSizing(crossCouplePairs,pulldowns,transistorWidths(1:25),margin);
