% Definition of a PASSGATE_FF_SCAN with PASSGATE_LATCH element and
% PASSGATE_LATCH_SCAN elements, is a passgate flip-flop where the slave
% slave stage (PASSGATE_LATCH_SCAN) has additional logic gates to simulate
% scan circuitry. 
%
...
%
% To create a PASSGATE_FF_SCAN, requires
% 1. name: passgate flip-flop name
% 2. wid: circuit width, wid(1:10)  is for the master stage
%                        wid(11:24) is for the slave stage
% 3. rlen: relative circuit length, use 1 by default.
% E.g. pgFF = PASSGATE_FF_SCAN('pgFF_0',450e-7,1) creates a passgate flip-flop
% where all the transistor widths are 450nm.
classdef PASSGATE_FF_SCAN < circuit
    ...
    methods
        function this = PASSGATE_FF_SCAN(name,wid,rlen)
            ...
            pgl_slave = PASSGATE_LATCH_SCAN('PGL_1',widSlave,rlenSlave); this.add_element(pgl_slave);
