function [b,err] = lft_quad_pt(model,bbox,lp,useErr,useDirectVertices)
% [b,err] = lft_quad_pt(model,bbox,lp,useErr,useDirectVertices)
...
%
% That is, for a point (x1,...xn), it is normalized to the middle grid firstly
...
%
% Input 
%  model:  N-n quadratic function (from ccm_getModel function). Requires the following fields:
...
%  useErr: add the quadratic arpproximation error
%  useDirectedVertices: 1 by default for performance. 
%          To find the bound of err, extreme points are computed. Some of them are cube vertices, with
%          this optional enabled, solving linear systems for these vertertices is skipped.
%
% Output
...
if(nargin<5||isempty(useDirectVertices))
  useDirectVertices = true;
end
...
for i=1:n
    ...
    S(i,:) = reshape(repmat(reshape((bboxm(i,1):bboxm(i,2))+round(model.GRID.v0(i)/model.GRID.dv(i)),SIZ2),SIZ),1,[]); % v0/dv==-1 in matlab
...
if(~isempty(lp))
    ...
    nc = size(L,2);
    if(nc==0)
        error('intersection of lp and bbox is empty');
    end
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Compute the error
%  The basic idea is to find the maximum error of each cell.
%  In a cell, the max error may occur on extreme points or boundary.
%  The extreme point is where the derivative is zero.
%  On the n-1 n boundary, we should also find its extreme poiont, similary for n-k n boudnary
%  There are choice^n points should be checked, (choice-1)^i*nchoosek(n,i) points on the n-i n face
%  For de/dxi, there are three choices:
%    de/dxi = 0 if xi is not fixed variable of the boudary
%    xi = L if xi is fixed low value on the boundary
%    xi = H if xi is fixed high value on the boundary
%  Therefore, we set choice = 3;

choice = 3; np = choice^n; % # of critical points

% First, find the extreme point in the cube
% e = f - ff => df/dxi = dff/dxi = bi
%  df/dxi = sum(sum( (d(E*E')/dxi).*B )) = dv(i) * sum(sum( (ei*e1'+e1*ei') .*B ))
% That is , we use the ith row and ith col of B.
% However, model only store lower triangle of B, therefore, we use (i,1:i),(i+1:n+1,:i) of B
% For example, in 3D case
%       |2b1,b2,b3,b4;|  |x1|   |dv(1)*b1|
%  df/dx= |b2,2b5,b6,b7;|* |x2| = |dv(2)*b2|
%         |b3,b6,2b8,b9;|  |x3|   |dv(3)*b3|
%       |b4,b7,b9,2b10|  |1 |   |    1   | % not use
% Then we solve x = A\C

% generate the symmetric indices matrix of A
pos = zeros(n+1,n+1);
pos(qpind) = 1:k;
pos = pos+pos'-diag(diag(pos));
posl = pos(1:n,1:n); posr = pos(1:n,end); % move constant term to the right side

% prepare the linear system: posl.*(ones+eye)*x = dv*b - posr
C = repmat(model.GRID.dv.*b(1:n),1,nc)  - Bs(posr,:); % each column for one linear system
A = Bs(posl,:).*repmat(reshape(1+eye(n),[],1),1,nc);
A = reshape(A,n,[]); % each nxn block for a linear system

% Then, we want to find extreme point on the boundary
% For example, on the x1 = lo face, we can solve df/dx = b similarly
% The difference is that x1 is fixed as the value of lo.
% Therefore, we should repalce the constraint of df/dx1 with x1 = lo
% and solve the linear system for the value of x2 x3
% Generally, for n-k face, we should replace k constraints with fixed value.
% That is, for each row of C, we have three choices: 1) the one above, 2) lo value 3) hi value
% A has three corresponding choices, 1) the one above 2) <ei> 3) <ei>
% Therefore, we should generate all choice^n linear systems and find all extreme points
%
% compute all choices
C = [C,L,H]; % three possible choices of C: critical point, low and high
A = [A,repmat(eye(n),1,2*nc)]; % three possible choice of A: critical piont, eye and eye.
cC = mat2cell(C,ones(1,n),ones(1,choice)*nc); % cell array. a cell for a variable and nc cubes.
cA = mat2cell(A,ones(1,n),ones(1,choice)*n*nc);

% compute index of all combinations
pInd = mod(floor(  repmat((0:(np-1)),n,1)...
                   ./repmat(choice.^(((n-1):-1:0)'),1,np) ), choice );
if(useDirectVertices) % use cube vertices directly for performance
    nvInd = (prod(pInd,1)==0); % non-vertex index
    vInd = ~nvInd; % vertex index
    nv = (choice-1)^n; nnv = np - nv; % number of non-vertex, vertex point per cell
end
pInd = sub2ind([n,choice],repmat((1:n)',1,np),pInd+1);

% generate all linear systems
CC = cell2mat(cC(pInd)); % 1xnp blocks, each block is nxnc matrix, the col of matrix is C vector for one point of a cell
AA = cell2mat(cA(pInd)); % 1xnp blocks, each block is nxn*nc matrix, the nxn block is A matrix for one point of a cell

% compute all critical points
if(useDirectVertices)
    nvAA = AA(:,reshape(repmat(nvInd,n*nc,1),1,[]));
    nvCC = CC(:,reshape(repmat(nvInd,nc,1),1,[]));
    nvx  = utils_solveLS(nvAA,nvCC); % critical points by solving linear systems
    vx   = CC(:,reshape(repmat(vInd,nc,1),1,[]));  % vertices
    x = [nvx,vx];
else
    x = utils_solveLS(AA,CC);
end

% At last, evaluate the error over all points
% e  = f - ff
% ff = b'*x
% f  = sum(sum( u*u' .* B))
%
u = [x;ones(1,nc*np)];
dv = reshape(repmat(u,n+1,1),n+1,[]).*repmat(reshape(u,[],1),1,n+1)'; % u*u'
dv = reshape(dv,(n+1)^2,[]);
dv = dv(qpind,:); % pickup lower triangular elements
realV = repmat(Bs,1,np).*dv;
realV = sum(realV,1);
xx = (x+repmat(S,1,np)).*repmat(model.GRID.dv,1,nc*np); % from cube position to original
estV = b(1:end-1)'*xx+b(end);
errFull = estV - realV;

% However, some points are not in the cube, remove them.
if(useDirectVertices)
    inCube = [( all(nvx >= repmat(L,1,nnv),1) & all(nvx <= repmat(H,1,nnv),1) ), true(1,nv*nc)];
else
    inCube = ( all(x >= repmat(L,1,np),1) & all(x <= repmat(H,1,np),1) );
end
errFull = errFull(inCube); % nn x 1 vector
% Find the maximum of all points of all cells
err = [min(errFull); max(errFull)];
